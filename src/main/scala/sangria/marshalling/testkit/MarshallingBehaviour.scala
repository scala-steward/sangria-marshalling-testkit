...
import sangria.marshalling.{InputUnmarshaller, ResultMarshaller, ScalaInput}
...
trait MarshallingBehaviour {
  ...
  def `value (un)marshaller`[T](rm: ResultMarshaller)(implicit
      iu: InputUnmarshaller[rm.Node]): Unit = {
    ...
    "(un)marshal list values" in {
      ...
      val seq = Vector(
        map,
        rm.nullNode,
        rm.scalarNode("ABC", "Test", Set.empty),
        rm.arrayNode(Vector.empty))
    ...
    "(un)marshal map values" in {
      ...
      val marshaled2 = rm.mapNode(rm.addMapNodeElem(
        rm.addMapNodeElem(rm.emptyMapNode(Seq("first", "second")), "first", seq, optional = false),
        "second",
        rm.nullNode,
        optional = false))
      val marshaled3 = rm.mapNode(
        rm.addMapNodeElem(
          rm.addMapNodeElem(rm.emptyMapNode(Seq("first", "second")), "first", seq, optional = true),
          "second",
          rm.nullNode,
          optional = true))
      ...
      List(marshaled1, marshaled2, marshaled3).foreach { marshaled =>
        marshaled.convertMarshaled[Any @@ ScalaInput] should be(
          Map("first" -> Vector(Map("a" -> 1), null, "ABC", Vector.empty), "second" -> null))
    ...
    "marshal optional list values" in {
      rm.optionalArrayNodeValue(Some(rm.scalarNode(123, "Test", Set.empty))) should be(
        rm.scalarNode(123, "Test", Set.empty))
